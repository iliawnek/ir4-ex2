...
public class AveragePositionDifferenceDSM extends DependenceScoreModifier {
	...
	{
		// get all posting lists for this document
        List<IterablePosting> relevantPostingLists = new ArrayList<>();
		for (int i = 0; i < okToUse.length; i++) {
		    if (okToUse[i]) relevantPostingLists.add(ips[i]);
        }

        // return if not enough terms to calculate proximity
        if (relevantPostingLists.size() < 2) return 0.0d;

        // compute average position for each query term
        List<Double> averagePositions = new ArrayList<>();
		for (IterablePosting ip : relevantPostingLists) {
            int[] positionVector = ((BlockPosting) ip).getPositions();
            double sum = 0.0d;
